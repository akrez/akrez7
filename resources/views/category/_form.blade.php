...
<x-form :method="isset($category) ? 'PUT' : 'POST'" :action="isset($category) ? route('categories.update', ['id' => $category['id']]) : route('categories.store')">
    ...
    <x-input :md="$isVertical ? 3 : 12" :row="!$isVertical" name="category_status" :errors="$errors"
        :value="isset($category) ? \Arr::get($category, 'category_status.value') : \App\Enums\CategoryStatusEnum::ACTIVE->value" type="select"
        :options="\App\Enums\CategoryStatusEnum::toArray()" :label="__('validation.attributes.status')" />
