...
<x-form :method="isset($gallery) ? 'PUT' : 'POST'" :action="isset($gallery) ? route('galleries.update', ['id' => $gallery['id']]) : route('galleries.store')">
    ...
    @endif
    @if (!isset($gallery))
        <x-input :md="$isVertical ? 3 : 12" :row="!$isVertical" type="file" name="file" :errors="$errors"
    ...
    @endif
    <x-button-submit :md="$isVertical ? 3 : 12" name="submit" :class="isset($gallery) ? 'btn-primary' : 'btn-success'">
